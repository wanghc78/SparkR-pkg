...
setup <- function(args=c('local', '24', '1000000', '10', '50')) {
    ...
    cat('[INFO][', app.name, ' on ', master, '] slices=', slices, ', n=', n, ', ndim=', ndim, ', niter=', niter, '\n', sep='')
    ...
    YX <- lapplyPartition(rdd, function(elems){
                ...
                X<- matrix(runif(local_n*ndim, 0, 10), nrow=ndim, ncol=local_n) 
                Y<- colSums(X) + rnorm(local_n) + 1 # now the coefficient are all 1
